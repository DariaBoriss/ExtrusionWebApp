...
int main() {
    ...
    // === ЯВНО ОТДАЁМ chart.min.js ===
    svr.Get("/chart.min.js", [&](const httplib::Request&, httplib::Response& res) {
        std::ifstream file("web/chart.min.js", std::ios::binary);  // БЕЗ ./
        if (!file.is_open()) {
            res.status = 404;
            res.set_content("404: chart.min.js not found in web/", "text/plain");
            return;
        }
        std::vector<char> buffer((std::istreambuf_iterator<char>(file)), {});
        res.set_content(buffer.data(), buffer.size(), "application/javascript");
        });
